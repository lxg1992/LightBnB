...
const getAllReservations = function(guest_id, limit = 10) {
  return pool.query(`SELECT properties.*, reservations.*, avg(rating) as average_rating
    ...
    LIMIT $2;`,[guest_id, limit]).then(res => res.rows).catch(err => console.log(err));
}
...
const getAllProperties = function(options, limit = 10) {
  ...
  const whereOrAnd = function(where) {
    if (where) {
      isWhere = false;
      return "WHERE";
    } else {
  ...
  if (options.city) {
    ...
    queryString += `${whereOrAnd(isWhere)} city LIKE $${queryParams.length} `;
  }

  if(options.owner_id) {
    queryParams.push(`${options.owner_id}`);
    queryString += `${whereOrAnd(isWhere)} owner_id = $${queryParams.length} `;
  }

  if(options.minimum_price_per_night) {
    queryParams.push(options.minimum_price_per_night * 100);
    queryString += `${whereOrAnd(isWhere)} cost_per_night >= $${queryParams.length} `; 
  }
  ...
  if(options.minimum_rating) {
    ...
    queryString += `${whereOrAnd(isWhere)} rating >= $${queryParams.length} `;
  }
...
const addProperty = function(property) {
  ...
  `,[owner_id, title, description, thumbnail_photo_url, cover_photo_url, cost_per_night, street, city, province, post_code, country, parking_spaces, number_of_bathrooms, number_of_bedrooms]).then(res => {
    return res.rows;
  }).catch(err => console.log(err));
